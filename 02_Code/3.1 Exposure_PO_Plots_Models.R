...
# Infer exposure_scale from term (iqr_ prefix) if not present
if (!"exposure_scale" %in% names(models_logit)) {
  models_logit <- models_logit |>
...
# Supports both raw and iqr terms (tot_/iqr_tot_, w20_/iqr_w20_, t1_/iqr_t1_, etc.)
...
# Prepare plot data for both scales (called inside loop per scale)
get_plot_data_by_scale <- function(models_df, scale) {
  prepare_plot_data(models_df) |>
    dplyr::filter(exposure_scale == scale)
}

# Ensure log columns exist (compute from OR/HR if missing, e.g. from older Excel)
ensure_log_columns <- function(plot_data_logit, plot_data_cox) {
  ...
  list(logit = plot_data_logit, cox = plot_data_cox)
}
...
for (scale in exposure_scales) {
  ...
  plot_data_logit <- get_plot_data_by_scale(models_logit, scale)
  plot_data_cox  <- get_plot_data_by_scale(models_cox, scale)
  log_cols <- ensure_log_columns(plot_data_logit, plot_data_cox)
  plot_data_logit <- log_cols$logit
  plot_data_cox  <- log_cols$cox

  # Suffix for filenames: _raw or _iqr
  scale_suffix <- if (scale == "raw") "_raw" else "_iqr"
  scale_key    <- if (scale == "raw") "raw" else "iqr"
  ...
  for (cont in contaminants) {
    for (tp in types) {
      ...
      if (length(plots_logit_log) > 0) {
        ...
        list_log_scale[[paste0("logit_", cont, "_", tp, "_", scale_key)]] <- fig_logit_log

        outfile <- sprintf("03_Output/Models/Plots_logit/OR_%s_%s%s.png", cont, tp, scale_suffix)
        ggsave(outfile,
          plot = fig_logit_log,
          res = 300,
          width = 20,
      ...
      if (length(plots_logit_or) > 0) {
        ...
        list_ratio_scale[[paste0("logit_", cont, "_", tp, "_", scale_key)]] <- fig_logit_or

        outfile <- sprintf("03_Output/Models/Plots_logit/OR_%s_%s_ratio%s.png", cont, tp, scale_suffix)
        ggsave(outfile,
          plot = fig_logit_or,
          res = 300,
          width = 20,
      ...
      if (length(plots_cox_log) > 0) {
        ...
        list_log_scale[[paste0("cox_", cont, "_", tp, "_", scale_key)]] <- fig_cox_log

        outfile <- sprintf("03_Output/Models/Plots_cox/HR_%s_%s%s.png", cont, tp, scale_suffix)
        ggsave(outfile,
          plot = fig_cox_log,
          res = 300,
          width = 20,
      ...
      if (length(plots_cox_hr) > 0) {
        ...
        list_ratio_scale[[paste0("cox_", cont, "_", tp, "_", scale_key)]] <- fig_cox_hr

        outfile <- sprintf("03_Output/Models/Plots_cox/HR_%s_%s_ratio%s.png", cont, tp, scale_suffix)
        ggsave(outfile,
          plot = fig_cox_hr,
          res = 300,
          width = 20,
