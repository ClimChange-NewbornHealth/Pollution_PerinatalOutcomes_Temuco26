...
table_data_logit <- get_table_data_by_scale(models_logit, "raw")
...
## 6. IQR version (same structure, exposure scaled by IQR) ----

table_data_logit_iqr <- get_table_data_by_scale(models_logit, "iqr")
table_data_cox_iqr  <- get_table_data_by_scale(models_cox, "iqr")

tab_or_iqr <- build_table_all(table_data_logit_iqr)
tab_hr_iqr <- build_table_all(table_data_cox_iqr)

writexl::write_xlsx(
  list(OR = tab_or_iqr, HR = tab_hr_iqr),
  path = "03_Output/Models/Tables/Tab_Exposure_PO_IQR.xlsx"
)
message("Saved: 03_Output/Models/Tables/Tab_Exposure_PO_IQR.xlsx")
