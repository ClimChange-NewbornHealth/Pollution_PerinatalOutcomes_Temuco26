...
# Pivot weekly exposure columns to long: idbase, week, exposicion, contaminante, tipo
data_long <- data |>
  pivot_longer(
...
for (contam in contaminants) {
  for (tipo_val in types) {
    ...
    for (dep_var in dependent_vars) {
      ...
      key <- paste(dep_var, contam, tipo_val, sep = "_")
...
message("Saved: 03_Output/DLM/DLM_PO_results.RData")
